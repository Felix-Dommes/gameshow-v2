...
impl DataHandler {
	...
	/// Normally the worker finishes all accesses and then terminates when all
	/// DataHandlers are dropped/closed.
	pub fn new() -> Self {
	...
	pub async fn create_player(&self, name: String) -> Result<String, &'static str> {
		...
		self.sender
			.send(DataAccess::CreatePlayer(result_sender, name))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
			.map_err(|_err| "db receive data access result error (dropped channel)")
	}
	...
	pub async fn set_player_name(&self, uuid: String, name: String) -> Result<bool, &'static str> {
		let (result_sender, result_receiver) = oneshot::channel();
		self.sender
			.send(DataAccess::SetPlayerName(result_sender, uuid, name))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
	...
	pub async fn get_player_name(&self, uuid: String) -> Result<Option<String>, &'static str> {
		let (result_sender, result_receiver) = oneshot::channel();
		self.sender
			.send(DataAccess::GetPlayerName(result_sender, uuid))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
	...
	pub async fn create_lobby(
		...
		let (result_sender, result_receiver) = oneshot::channel();
		self.sender
			.send(DataAccess::CreateLobby(result_sender, admin_uuid, admin_name))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
	...
	pub async fn get_lobby(&self, uuid: String) -> Result<Option<Arc<Gameshow>>, &'static str> {
		let (result_sender, result_receiver) = oneshot::channel();
		self.sender
			.send(DataAccess::GetLobby(result_sender, uuid))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
	...
	pub async fn set_question_sets(
		&self,
		question_sets: Vec<(String, PathBuf)>,
	) -> Result<(), &'static str> {
		let (result_sender, result_receiver) = oneshot::channel();
		self.sender
			.send(DataAccess::SetQuestionSets(result_sender, question_sets))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
	...
	pub async fn get_question_sets(&self) -> Result<Vec<(String, PathBuf)>, &'static str> {
		let (result_sender, result_receiver) = oneshot::channel();
		self.sender
			.send(DataAccess::GetQuestionSets(result_sender))
			.await
			.map_err(|_err| "db send data access error (dropped channel)")?;
		result_receiver
			.await
